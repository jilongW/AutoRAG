...
class Evaluator:
    ...
    def restart_trial(self, trial_path: str):
        ...
        node_names = list(map(lambda node: list(map(lambda n: n.node_type, node)), nodes))
        ...
        remain_nodes, completed_node_names, remain_lines, remain_line_names \
            = self._set_remain_nodes_and_lines(node_line_names, nodes, node_names,
                                               conflict_node_name, conflict_line_name)
        # Set previous_result
        previous_result = self.__set_previous_result(node_line_names, node_names, trial_path, conflict_node_name)
        ...
        trial_summary_df = pd.DataFrame(columns=['node_line_name', 'node_type', 'best_module_filename',
                                                 'best_module_name', 'best_module_params', 'best_execution_time'])
        completed_line_names = node_line_names[:node_line_names.index(conflict_line_name)]
        ...
        if completed_line_names:
            for line_name in completed_line_names:
                ...
                trial_summary_df = self._append_node_line_summary(line_name, node_line_dir, trial_summary_df)
        if remain_lines:
    ...
    def __find_conflict_point(self, trial_path: str, node_line_names: List[str],
        ...
        for node_line_name in node_line_names:
            ...
            if not os.path.exists(os.path.join(node_line_dir, 'summary.csv')):
                conflict_node_name = self._find_conflict_node_name(node_line_dir, node_lines[node_line_name])
    ...
    def _find_conflict_node_name(node_line_dir: str, node_line: List[Node]) -> str:
        for node in node_line:
            ...
            if not os.path.exists(node_dir) or not os.path.exists(os.path.join(node_dir, 'summary.csv')):
                return node.node_type
        raise TypeError("No conflict node name found.")

    def __set_previous_result(self, node_line_names: List[str], node_names: List[List[str]],
                              trial_path: str, conflict_node_name: str):
    ...
    @staticmethod
    def _set_remain_nodes_and_lines(node_line_names: List[str], nodes: List[List[Node]],
                                    node_names: List[List[str]], conflict_node_name: str,
        ...
        if conflict_node_name == full_conflict_node_line_node_names[0]:
            ...
            conflict_node_index = full_conflict_node_line_node_names.index(conflict_node_name)
            remain_nodes = full_conflict_node_line_nodes[conflict_node_index:]
            completed_node_names = full_conflict_node_line_node_names[:conflict_node_index]
        ...
        return remain_nodes, completed_node_names, remain_node_lines, remain_node_line_names

    @staticmethod
    def _append_node_line_summary(node_line_name: str, node_line_dir: str, trial_summary_df: pd.DataFrame):
    ...
    @staticmethod
    def _append_node_summary(node_line_dir: str, node_name: str, summary_lst: List[Dict]):
        node_summary_df = load_summary_file(os.path.join(node_line_dir, node_name, 'summary.csv'))
